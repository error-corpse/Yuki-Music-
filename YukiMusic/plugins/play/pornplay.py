...
from  YukiMusic.plugins.play import play
from  YukiMusic.plugins.play.pornplay import play
...
def get_video_info(title):
    url_base = f'https://www.hqporner.com/search/{title}'
    try:
            ...
            if video_list:
                ...
                if thumbnail:
                    ...
                    if link and 'https://' not in link:  # Check if the link is a valid video link
                        return {'link': 'https://www.hqporner.com' + link, 'thumbnail': thumbnail_500}
...
@app.on_message(filters.command("hqporner"))
async def get_random_video_info(client, message):
    if len(message.command) == 1:
        await message.reply("Please provide a title to search.")
        return
